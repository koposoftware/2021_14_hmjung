...
<script>
	$(document).ready(function() {
		...
		$('#goback').click(function(){
			location.href = "${pageContext.request.contextPath}/account/advertise/requestadcheck"
		})	
	})
												...
												<div class="table-responsive">
													<table class="table table-striped table-hover no-margin">
														<tbody>
															<tr>
																<td>회사 종목 코드</td>
																<td><a href="#">${adRequestVO.advertiseSymbol }</a></td>
															</tr>
															<tr>
																<td>광고 날짜</td>
																<td><a href="#">${adRequestVO.startDate } ~ ${adRequestVO.endDate } </a></td>
															</tr>
															<tr>
																<td>빈도수</td>
																<td>${adRequestVO.frequency } 시간 간격</td>
															</tr>
															<tr>
																<td>요청 결과 상태</td>
																 <c:if test="${adRequestVO.status eq  'WAIT'}">
																	<td><span class="badge badge-warning">확인 되지 않음</span></td> 
																 </c:if>
																 <c:if test="${adRequestVO.status eq  'APPROVE'}">
																	<td><span class="badge badge-success">승인</span></td> 
																 </c:if>
																  <c:if test="${adRequestVO.status eq  'DISPROVE'}">
															...
															<tr>
																...
																<td><form method="get" action="${ pageContext.request.contextPath }/ajax/filedown/downloadFile">
																			<input hidden="true" name="serverPath" value="${ adRequestVO.filePath}">
												...
												<div>
													<div class="row justify-content-between">
														<div class="col-md-auto">
															<h5>${adRequestVO.note }</h5>
														</div>
														<div class="col-md-auto">
															<h5 class="text-end">${adRequestVO.time }</h5>
														</div>
													</div>
